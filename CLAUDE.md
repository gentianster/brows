...
├── ipc.rs        常駐インスタンスへの URL 転送（名前付きパイプ \\.\pipe\brows.<SID>）
...
└── ui/
    ...
    ├── tray.rs     タスクトレイアイコン（常駐中の設定画面への導線）
...
- シングルインスタンス判定は名前付きパイプの作成成否で行う（`FILE_FLAG_FIRST_PIPE_INSTANCE`。作成成功 = 常駐になる）。パイプ名にユーザー SID を含めてユーザーごとに分離し、DACL を本人 + SYSTEM のフルアクセスに限定する
- `--resident` でウィンドウを表示せず常駐だけ開始できる（設定画面の「Windows 起動時に常駐する」で HKCU の Run キーに登録）
- どの起動経路でもトレイ常駐を確保する。引数なし起動（ダブルクリック）は設定画面を開きつつ
  裏で `--resident` を spawn し、自動更新後の再起動も `--resident` で行う。`--resident` は冪等
  （既に常駐がいれば `try_bind` 失敗で即終了）なので二重常駐にならない。ただし管理者権限
  （`--register` 直後など）では昇格した常駐を作らないよう `util::is_elevated()` で抑止する
...
- 常駐になると `tray.rs` がタスクトレイにアイコンを出す。eframe（winit）とは別スレッドで
  非表示ウィンドウ＋メッセージループを持ち、`Shell_NotifyIconW` で登録する。左クリックで
  設定画面（別プロセス）を開き、右クリックで「設定を開く／常駐トグル」メニューを出す
- トレイのゴーストアイコンを残さないよう、終了経路（`BROWS-EXIT` 受信時）で `tray::cleanup()`
  を呼んでから `exit` する。Explorer 再起動時は `TaskbarCreated` を受けてアイコンを再登録する
